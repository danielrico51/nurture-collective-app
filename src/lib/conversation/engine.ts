...
const buildChatMessages = (session: ConversationSession) => [
  { role: "system" as const, content: CONCIERGE_SYSTEM_PROMPT },
  {
    ...
    content: `Current extracted profile (background): ${JSON.stringify(session.extractedProfile)}`,
  },
  ...session.messages
...
const fallbackAssistantReply = (
  ...
  if (!profile.name || !profile.phone) {
    return {
      content:
        "Almost there. Could you share your name and the best phone number for your care coordinator to reach you?",
      quickReplies: [],
    };
  }
